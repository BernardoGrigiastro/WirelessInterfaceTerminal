...
public class ItemWIT extends ItemWT implements IWirelessInterfaceTerminalItem, IBaubleItem {
	...
	public ActionResult<ItemStack> onItemRightClick(final World world, final EntityPlayer player, final EnumHand hand) {
		...
		if (world.isRemote && hand == EnumHand.MAIN_HAND && !item.isEmpty() && getAECurrentPower(item) > 0) {
			...
			if (!WITApi.instance().isTerminalLinked(item)) {
	...
	public void addCheckedInformation(final ItemStack is, final World world, final List<String> list, final ITooltipFlag advancedTooltips) {
		...
		if (WTApi.instance().getConfig().isInfinityBoosterCardEnabled()) {
			if (WTApi.instance().getConfig().isOldInfinityMechanicEnabled()) {
				...
				final String infinityEnergyString = WITUtils.isWITCreative(is) ? I18n.format(WTApi.instance().getConstants().getTooltips().infinite()) : isShiftKeyDown() ? "" + infinityEnergyAmount + "" + TextFormatting.GRAY + " " + I18n.format(WTApi.instance().getConstants().getTooltips().units()) : ReadableNumberConverter.INSTANCE.toSlimReadableForm(infinityEnergyAmount);
